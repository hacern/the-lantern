# The Lantern — Article Generator Prompt (v3.4)
...
### Site chrome (mandatory on every generation)

Every article HTML file **must** include a visible, working link to the site home:

`https://the-lantern.hacernube.workers.dev/`

* Place it in the **header** (site name / wordmark “The Lantern”) **and** repeat it in the **footer**. Both must use that exact absolute URL as `href` — do not use `#`, `index.html`, a relative `../`, or a placeholder.
* The control is a real `<a>` (not a `<span>` or JS-only click). Visible text or `aria-label` is `The Lantern` or `Home`.
* Required in `FULL` and `FAST`, `multimodal` and `text-only`. Isolated-file opens still resolve home because the URL is absolute.
* Do not omit the header to look “cinematic.” Navigation is not optional chrome.
...
6. **Home-link Check (hard fail):** The HTML contains at least two `<a href="https://the-lantern.hacernube.workers.dev/">` elements (header + footer). Reject relative-only home links, `#`, missing header, or home text that is not an actual anchor.
